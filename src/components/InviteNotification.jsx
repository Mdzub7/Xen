...
const InviteNotification = () => {
  ...
  console.log("InviteNotification User:", user); // <--- Add this

  useEffect(() => {
    if (!user) {
      console.log("No user, skipping invite fetch."); // <--- Add this
      return;
    }

    console.log("Fetching invites for user:", user.uid); // <--- Add this
    const userRef = doc(db, "users", user.uid);
    const unsubscribe = onSnapshot(userRef, (docSnap) => {
      if (docSnap.exists()) {
        const invites = docSnap.data().invites || [];
        setInvites(invites);
        console.log("Invites updated:", invites); // <--- Add this
      } else {
        console.log("User document does not exist or has no data."); // <--- Add this
      }
    });

    return () => {
      console.log("Unsubscribing from invite listener."); // <--- Add this
      unsubscribe();
    };
  }, [user]);
